...
class camera {
  public:
    ...
    void render_scanlines(int start, int end, const hittable& world, const hittable& lights, std::vector<color>& frame_buffer) {
        for (int j = start; j < end; j++) {
            {
                std::lock_guard<std::mutex> guard(clog_mutex);
                std::clog << "\rProgress: " << 100 * ++scanlines_progress / image_height << '%' << std::flush;
            }
    ...
    void render(const hittable& world, const hittable& lights) {
        ...
        for (int t = 0; t < num_threads; ++t) {
            ...
            threads.emplace_back(&camera::render_scanlines, this, start, end, std::ref(world), std::ref(lights), std::ref(frame_buffer));
        }
